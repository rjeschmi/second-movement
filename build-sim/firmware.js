...
var ASM_CONSTS = {
  80104: () => { return Module['suspended']; },  
 80136: ($0) => { Module['suspended'] = $0; },  
 80166: () => { return window.ir_adc_value || 6500; },  
 80206: () => { var len = lengthBytesUTF8(tx) + 1; var s = _malloc(len); stringToUTF8(tx, s, len); return s; },  
 80303: () => { tx = ""; },  
 80316: ($0, $1) => { const classList = document.querySelector('#btn' + $0).classList; const highlight = 'highlight'; $1 ? classList.add(highlight) : classList.remove(highlight); },  
 80477: () => { return new Date().getTimezoneOffset() * 60 * 1000; },  
 80532: ($0) => { const date = new Date(Date.now() + $0); return date.getSeconds() | (date.getMinutes() << 6) | (date.getHours() << 12) | (date.getDate() << 17) | ((date.getMonth() + 1) << 22) | ((date.getFullYear() - 2020) << 26); },  
 80750: ($0) => { window.sim_rtc_counter = $0; if (window.ir_pb_frame && window.ir_pb_start >= 0) { var bi = Math.floor(($0 - window.ir_pb_start) / window.ir_pb_tpb); if (bi >= 0 && bi < window.ir_pb_frame.length) { window.ir_adc_value = window.ir_pb_frame[bi] === 1 ? 4200 : 6500; if (bi !== window.ir_pb_last_bi) { console.log('IR bit ' + bi + ': counter=' + $0 + ' elapsed=' + ($0 - window.ir_pb_start) + ' tpb=' + window.ir_pb_tpb + ' val=' + (window.ir_pb_frame[bi] === 1 ? 'BRIGHT' : 'dark')); window.ir_pb_last_bi = bi; } } else { window.ir_adc_value = 6500; window.ir_pb_start = -1; } } },  
 81331: () => { document.getElementById("custom").style.display = ""; },  
 81387: () => { document.getElementById("classic").style.display = "none"; },  
 81448: () => { document.getElementById("classic").style.display = "none"; },  
 81509: () => { document.getElementById("custom").style.display = "none"; },  
 81569: ($0, $1) => { document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 1); },  
 81687: ($0, $1) => { document.querySelectorAll("[data-com='" + $0 + "'][data-seg='" + $1 + "']") .forEach((e) => e.style.opacity = 0); },  
 81805: () => { document.querySelectorAll("[data-com][data-seg]") .forEach((e) => e.style.opacity = 0); },  
 81897: () => { if (!Module['audioContext']) { Module['audioContext'] = new (window.AudioContext || window.webkitAudioContext)(); } },  
 82017: ($0) => { const audioContext = Module['audioContext']; if (!audioContext) return; if (!(audioContext._oscillator && audioContext._gain)) { const oscillator = audioContext.createOscillator(); const gain = audioContext.createGain(); oscillator.type = 'triangle'; oscillator.connect(gain); gain.connect(audioContext.destination); oscillator.start(0); audioContext._oscillator = oscillator; audioContext._gain = gain; } audioContext._oscillator.frequency.value = 1e6/$0; audioContext._gain.gain.value = volumeGain; },  
 82522: () => { const audioContext = Module['audioContext']; if (audioContext && audioContext._gain) { audioContext._gain.gain.value = 0; } },  
 82650: ($0, $1, $2) => { let filter = document.getElementById("ledcolor"); let color_matrix = filter.children[0].values.baseVal; color_matrix[0].value = $0 / 255; color_matrix[6].value = $1 / 255; color_matrix[12].value = $2 / 255; document.getElementById('light').style.opacity = Math.min(255, $0 + $1 + $2) / 255; },  
 82945: () => { return lat; },  
 82961: () => { return lon; },  
 82977: () => { return temp_c || 25.0; },  
 83004: () => { return -new Date().getTimezoneOffset(); }
};
